...
{
    public Fill(Pen p, Tools tool, int size, Color color, Point start, Point end, Graphics g, Bitmap bmp) : base(p,
        tool, size, color, start, end, g, bmp)
    {
    }
    ...
    public void FillObject(Bitmap bmp, int x, int y, Color newColor)
    {
        ...
        try
        {
            ...
            if (newColor.ToArgb() == oldColor.ToArgb()) return;
